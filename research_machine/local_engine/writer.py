...
_PER_SECTION_TIPS: Dict[str, str] = {
    ...
    "results": (
        ...
        "(3) VERBATIM DISCIPLINE: reuse each finding's original key phrases and terms — "
        "change only the connective tissue between findings, never the factual wording "
        "itself. Keep each [Author, Year] tag immediately adjacent to its claim. Claims "
        "are machine-verified against the source abstracts: heavy paraphrase fails "
        "verification, and NEVER invent findings, statistics, or citations not in the list. "
